...
import { HARDCODED_GALLERY_IMAGES } from '../data/hardcodedGallery';
import { cloudinaryImgSrcForGalleryThumb, cloudinaryImgSrcForLightbox } from '../utils/cloudinaryImgSrc';
...
const GallerySection = () => {
  ...
  const galleryImages = useMemo(() => {
    if (!isPublicCmsEnabled()) return HARDCODED_GALLERY_IMAGES;
    const fromApi = data?.galleryImages;
    if (Array.isArray(fromApi) && fromApi.length > 0) return fromApi;
    return HARDCODED_GALLERY_IMAGES;
  }, [data]);
  ...
  return (
    <>
          ...
          {galleryImages.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {galleryImages.map((img, index) => {
                ...
                const thumbSrc = cloudinaryImgSrcForGalleryThumb(img, isLarge);
                return (
                  <div
                    key={`${img}-${index}`}
      ...
      {lightboxImage && (
        ...
        >
          <img
            src={cloudinaryImgSrcForLightbox(lightboxImage)}
            alt="Gallery preview"
