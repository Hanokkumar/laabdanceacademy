...
const InstructorSection = () => {
  ...
  const instructors = useMemo(
    () => publicListForDisplay(data?.instructors, fallbackInstructors),
    [data]
  );
  ...
  return (
    <section className="py-20 lg:py-28 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={refGrid} className={cn('grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-4xl mx-auto', revealGrid)}>
          {instructors.map((instructor) => (
            <div
              key={instructor.id}
              className="group text-center reveal-stagger"
            >
              <div className="relative h-80 w-full overflow-hidden rounded-xl bg-[#f5f5f5]">
                <img
                  src={cloudinaryImgSrcForDisplay(instructor.image)}
                  alt={instructor.name}
              ...
              <div className="mt-4">
                ...
                {instructor.bio ? (
                  <p className="text-gray-600 font-dm-sans text-sm mt-3 leading-relaxed text-left max-w-md mx-auto">
                    {instructor.bio}
                  </p>
                ) : null}
              </div>
