...
function isNavActive(to, end, pathname) {
      ...
      <aside className="w-64 shrink-0 bg-[#111] text-white flex flex-col border-r border-white/10">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {navItems.map(({ to, label, icon: Icon, end }) => {
            const active = isNavActive(to, Boolean(end), pathname);
            return (
              <Link
                key={to}
                href={to}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg font-dm-sans text-sm transition-colors ${
