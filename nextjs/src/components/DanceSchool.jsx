...
import { publicListForDisplay } from '../utils/publicSiteContent';
...
const DanceSchool = () => {
  ...
  const categories = useMemo(
    () => publicListForDisplay(data?.danceSchoolCategories, danceSchoolCategories),
    [data]
  );
  const schoolItems = useMemo(
    () => publicListForDisplay(data?.danceSchoolItems, danceSchoolItems),
    [data]
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={refHead} className={cn('text-center mb-12', revealHead)}>
          ...
          <p className="text-gray-500 font-dm-sans text-base mt-4 max-w-2xl mx-auto">
            Hip hop, folk, Bollywood, locking, popping, breaking, and freestyle choreography — filter by style and find your class at LAAB in Navalur, Chennai.
          </p>
        </div>
          ...
          <div ref={refGrid} className={cn('lg:col-span-3', revealGrid)}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredItems.map((item) => (
                <div
                  key={item.id}
                  className="group relative overflow-hidden rounded-md bg-white cursor-pointer reveal-stagger border border-transparent transition-all duration-300 hover:border-primary/30 hover:shadow-md hover:shadow-primary/20"
                >
                  <img
                    src={cloudinaryImgSrcForGalleryThumb(item.image, false)}
                    alt={item.category ? `${item.category} at LAAB` : 'LAAB dance class'}
                    className="w-full h-64 object-cover transition-transform duration-500 group-hover:scale-110"
                  />
                  <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/45 transition-all duration-500 flex items-center justify-center">
